...
# Serializes the read-modify-write cycle around FOLDERS_CONFIG_PATH so two
# same-process callers (API request thread vs. the job worker thread) can't
# race and lose one caller's update. Same-process only, per this app's
# single-user-local-tool scope — does not protect against multiple OS
# processes.
